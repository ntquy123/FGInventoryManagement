...
SELECT MFSM.SHPPKG AS Shppkg
 ...
 WHERE 1=1          
   ...
   AND MFSM.WH_CODE = :pWhCode";
            ...
            var rows = await _amtContext.ExFactoryPickingHeaderRow
        ...
        public async Task<List<ExFactoryPickingLineRow>> GetExFactoryPickingLinesAsync(string shpPkg)
        {
            var sql = @"
SELECT MFSD.SHPPKG AS Shppkg
 ...
 WHERE 1=1
   ...
   AND MFSD.SHPPKG = :pShpPkg";
            ...
            var rows = await _amtContext.ExFactoryPickingLineRow
        ...
        public async Task<(IReadOnlyList<ExFactoryPickingLineRow> rows, string rtnCode, string rtnMsg)> ScanExFactoryPickingAsync(ExFactoryPickingScanRequest request)
        {
